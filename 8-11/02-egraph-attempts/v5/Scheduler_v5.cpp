...
{//whether ndDiv can be divisor for ndRoot
	const std::set<int>& setRootConePI = ndRoot.m_setnConePI;
	int nNotIn = 0;
...
void Scheduler::NewDSE()
{
	...
	for (int i = 1; i < m_nRun; i++)
	{
		...
		// E-graph: try to use result if it improves gate count
		{
			string sf = m_netlist.m_strBench + "_subnet_expr.txt";
			string of = m_netlist.m_strBench + "_subnet_opt.txt";
			ExportXmgForEgraphNumbered(Sub.m_net, sf);
			vector<string> results = CallEgraphOptimizer(sf, of);
			if (!results.empty()) {
				xmg_network egNet = RebuildXmgFromNumbered(of);
				int nEg = egNet.num_gates();
				int nSub = Sub.m_net.num_gates();
				cout << "  [E-graph] " << nEg << " gates (vs sub " << nSub << ")\n";
				if (nEg > 0 && nEg <= nSub) {
					// Write to .v, read back to verify
					string vf = m_netlist.m_strBench + "_egraph_test.v";
					write_verilog(egNet, vf);
					xmg_network egNet2;
					try {
						lorina::read_verilog(vf, verilog_reader(egNet2));
						cout << "  [E-graph] verify OK, " << egNet2.num_gates() << " gates\n";
						NewNet = egNet2.clone();
						cout << "  [E-graph] USING! gate count: " << nSub << " -> " << nEg << "\n";
						// skip resyn2 loop
						goto skip_resyn2;
					} catch (...) {
						cout << "  [E-graph] verify FAILED, falling back to resyn2\n";
					}
				}
			}
		}
