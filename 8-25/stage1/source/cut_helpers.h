                                            ...
                                            std::string const& filepath)
{
    ...
    while (getline(fin, line)) {
        ...
        if (first == 'P' && line.rfind("PI ", 0) == 0) {
            // 第 i 个 PI 行 = leaves[i]（正相位 signal）
            size_t i = nodes.size();
            if (i < leaves.size())
                nodes.push_back(xmg.make_signal(xmg.index_to_node(leaves[i])));
        }
        else if (first == 'P' && line.rfind("PI_COUNT", 0) != 0 && line.rfind("PI_MAP", 0) != 0) {
            root_flat = atoi(line.c_str() + 2);
        }
        else if (line.rfind("C0", 0) == 0) nodes.push_back(xmg.get_constant(false));
        else if (line.rfind("C1", 0) == 0) nodes.push_back(xmg.get_constant(true));
        else if (first == 'X' || first == 'M' || first == 'N') {
            vector<int> kids;
            istringstream iss(line);
            string tok;
    ...
    fin.close();
    if (root_flat < 0 || root_flat >= (int)nodes.size())
